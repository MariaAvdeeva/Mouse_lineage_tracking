...
%         end
    ...
    for lvd=1:size(dup,1)
        ...
        if (num_large == 0) | (num_large >1)
            disp('Daughter candidates or more than two descendants')
            disp('To');
            disp(to);
            disp('From');
            disp(from);
...
%             disp('From')
%             
%             disp(cell_labels_I_care_about2(from(i)));
%             fv = volumes2(cell_labels_I_care_about2(from(i)));
%             %[k,fv] = convhull(alpha_shape_for_each_label2{from(i),:}.Points);
%             disp('From_volume')
%             disp(fv);
%             if ~daughter_flag
%                 if (fv <= vol_thres)
%                     bad_matches = [bad_matches,from(i)];
%                     disp('Adding this daughter because other one is a good match')
%                 else
%                     daughter_flag = true;
%                     disp('Returning daughter flag because daughters are equally good matches');
%                 end
%             else
%                 if (fv > vol_thres)
%                     daughter_flag = false;
%                     disp('This is a good match')
%                     if i == 2
%                         bad_matches = [bad_matches,from(1)];
%                         disp('Adding another daughter because this one is a good match')                   
%                     end
%                 end
%             end
%         end
%         %if daughter candidates or more than two descendants
